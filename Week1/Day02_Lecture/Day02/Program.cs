...
grades.RemoveAll(g => g < 59.5f);

for (int i = 0; i < grades.Count; i++)
{
    Console.WriteLine((float)grades[i]);
}

void GetRandomColor(out ConsoleColor outColor)
{
...
bool PostFix(ref string hero) //hero is now an alias to the variable used when calling PostFix. In this case, hero is an alias to the spider variable.
{
...
void FillGradesList(ref List<float> grades)
{
    for (int i = 0; i < 10; i++)
    {
...
void CalculateStats(List<float> grades, out float min, out float max, out float avg)
{
    min = grades.Min();
    max = grades.Max();
    avg = grades.Average();
